...
function OnboardingRoot() {
  ...
  return (
    <EddReviewContext.Provider value={value}>
      <OnboardingPage />
    </EddReviewContext.Provider>
  );
}
      ...
      <Card className="mt-5">
        ...
        <div className="mt-3 rounded-xl bg-[#f5f5f5] p-4 text-xs text-muted-foreground leading-relaxed">
          <p className="font-medium text-foreground">
            Required for all merchants with previous card processing activity. (including PayPal and
  ...
  return (
    ...
    >
      <div className="grid md:grid-cols-3 gap-4">
        {["ACH", "Wire"].map((label) => {
          const active = types[label];
          return (
            <button
              type="button"
              key={label}
              onClick={() => setTypes((p) => ({ ...p, [label]: !p[label] }))}
      ...
      {types.ACH && (
        <Card className="mt-6">
          <div className="flex items-start justify-between gap-3 mb-4">
            <div>
              <h3 className="font-semibold text-sm">Bank ACH Configuration</h3>
              <div className="text-xs text-muted-foreground mt-0.5">Settlement destination</div>
            </div>
            <button
              type="button"
              onClick={() => setTypes((p) => ({ ...p, ACH: false }))}
          ...
          <div className="grid md:grid-cols-2 gap-x-5 gap-y-4">
            ...
            <Field label="Bank Currency" required>
              <Input defaultValue="USD" readOnly icon={CreditCard} />
            </Field>
...
function EddSendBack() {
  const { rejectedItems } = useContext(EddReviewContext);
  if (rejectedItems.length === 0) return null;
  return (
    <div className="mt-8 rounded-2xl border border-destructive/50 bg-surface p-5">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h3 className="font-semibold text-sm text-destructive">
            Due Diligence items rejected
          </h3>
          <p className="mt-1 text-xs text-muted-foreground max-w-2xl">
            {rejectedItems.join(", ")} {rejectedItems.length > 1 ? "were" : "was"} rejected. The
